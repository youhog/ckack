<template>
  <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6">
    ...
    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">新增、刪除或編輯指定區域下的房間號碼與戶別。房間數量過多，已啟用分頁載入。</p>

    <form @submit.prevent="addRoom" class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
      ...
      <input
        type="text"
        v-model="newRoom.household"
        placeholder="戶別 (選填, 例如: H1)"
        class="form-control"
        title="輸入戶別 (選填)"
      >
      <button
        type="submit"
        class="btn-primary md:col-start-4"
        :disabled="isSaving || !newRoom.zone_id || !newRoom.floor || !newRoom.room_number"
      >
        <span v-if="isSaving">
    ...
    <div v-else class="space-y-6">
      ...
      <div v-else class="space-y-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          ...
          >
            <span class="truncate" :title="`${room.dorm_zones.name} - ${room.floor}樓 ${room.household ? room.household + '戶 ' : ''}${room.room_number}`">
              <span class="text-xs text-slate-500 dark:text-slate-400">{{ room.dorm_zones.name }}</span><br>
              <span class="font-medium text-slate-800 dark:text-slate-100">
                {{ room.floor }}樓 {{ room.household ? room.household + '戶 ' : '' }}{{ room.room_number }}
              </span>
            </span>
            <button
...
const newRoom = ref({ zone_id: '', floor: '', room_number: '', household: '' })
...
const fetchRooms = async () => {
    ...
    if (filterRoomNumber.value) {
        const searchTerm = filterRoomNumber.value.trim();
        if (searchTerm) {
            // 使用 encodeURIComponent 確保特殊字符被正確編碼
            const encodedSearchTerm = encodeURIComponent(searchTerm);
            // 建構 PostgREST 'or' 參數，格式為 or=(column.operator.value,column.operator.value)
            const orFilterString = `room_number.ilike.%${encodedSearchTerm}%,household.ilike.%${encodedSearchTerm}%`;
    ...
    if (error) { // Line 182 左右
        loadError.value = error.message;
        showToast(`載入房間失敗: ${error.message}`, 'error');
        console.error("Fetch rooms error:", error); // 顯示詳細錯誤物件
    } else {
        rooms.value = data;
...
const addRoom = async () => {
  if (!newRoom.value.zone_id || !newRoom.value.floor || !newRoom.value.room_number || isSaving.value) return;
  isSaving.value = true;
  const floorCleaned = newRoom.value.floor.trim();
  const roomNumberCleaned = newRoom.value.room_number.trim();
  const householdCleaned = newRoom.value.household ? newRoom.value.household.trim() : null;

  if (!floorCleaned || !roomNumberCleaned) {
      showToast('樓層和房間號碼不能為空。', 'error');
  ...
  if (error) {
    if (error.code === '23505') {
        showToast('新增失敗: 此區域樓層已存在相同房間號碼。', 'error')
    } else {
    ...
    newRoom.value.household = '';
    showToast('房間新增成功', 'success');
    if (!filterZone.value || filterZone.value === newRoom.value.zone_id) {
         fetchRooms();
    }
  }
  isSaving.value = false;
...
const deleteRoom = async (room) => {
  ...
  if (confirm(`確定要刪除房間 ${room.dorm_zones.name} - ${room.floor}樓 ${room.household ? room.household + '戶 ' : ''}${room.room_number} 嗎？\n與此房間關聯的報告和床位分配將會遺失連結。`)) { //
    isSaving.value = true;
    ...
    if (error) {
      ...
      if(rooms.value.length === 1 && currentPage.value > 1) {
          currentPage.value--;
      }
      fetchRooms();
    }
...
/* 微調按鈕樣式使刪除按鈕更小 */
.btn-icon-danger-sm {
   @apply inline-flex items-center justify-center p-1.5 text-xs rounded-md font-medium transition-all duration-200 cursor-pointer bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-500/30 disabled:opacity-60 disabled:cursor-not-allowed;
}
</style>